...
- Last updated is the default Session order for new and migrated browser state; Manual remains an explicit opt-in.
- The Ungrouped create affordance creates and opens a Session outside every Workspace account.
...
Domain and Host tests cover durable Workspace moves, no-op and invalid anchors, restart recovery, full-order RPC responses, order frames, and one Workspace snapshot per Host-stream baseline. Runtime tests cover optimistic order, frame/response precedence, overlapping rejection rollback to Host-confirmed order, reconnect baselines, New Session target priority, and explicit Ungrouped blank creation and reuse. UI tests cover five-row folding, transient expansion reset, pruning persisted state after Workspace removal, one-time migration from the former manual default to Last updated, order-preserving mode switches, recent-update promotion, browser-local Ungrouped and flat-list drag persistence, hierarchy-free flat-row leading spacing, selected view indicators, expanded-section Workspace hit testing, an unclipped first insertion boundary, outside-list Workspace and Session drops, search collapse rules, and compact CSS dimensions. An isolated assembled Web scenario creates a Session from the Ungrouped row, proves that no Workspace account owns it, and verifies that the new row becomes selected.
