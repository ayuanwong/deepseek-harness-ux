...
import {
  artifactDeliverableReferences, basename, deliverablePaths, deliverablesDefinition,
  producedFileMentions, producedForClosing, selectProducedFiles,
...
function tailOwner(
  ...
  return { seq, closingText, openFile, turn: turnLocation(turn, data) }
}
...
describe('produced-file Turn data', () => {
  ...
  it('adds exact binary artifact references from the closing answer without guessing prose paths', () => {
    const markdown = [
      'Files: `output/report.pdf`, `slides/deck.pptx`, `tables/data.xlsx`, and `media/preview.png`.',
      'Ignore plain notes/mentioned.pdf and URLs such as `https://example.com/report.pdf`.',
      'Ignore commands like `open output/other.pdf`, quoted `"output/quoted.pdf"`, and unsafe `../*.pdf`.',
      '```sh',
      'open output/fenced.pdf',
      '````',
      'Duplicate: `output/report.pdf`.',
    ].join('\n')

    expect(artifactDeliverableReferences(markdown)).toEqual([
      'output/report.pdf', 'slides/deck.pptx', 'tables/data.xlsx', 'media/preview.png',
    ])
    expect(deliverablePaths(tailOwner(
      produced([2, 'notes.md'], [3, 'output/report.pdf']), 9, () => {}, 1, markdown,
    ))).toEqual([
      'notes.md', 'output/report.pdf', 'slides/deck.pptx', 'tables/data.xlsx', 'media/preview.png',
    ])
  })

  it('recognizes other common OS-openable deliverables from exact closing references', () => {
    expect(artifactDeliverableReferences([
      '`data/results.csv` `design/layout.sketch` `model/scene.glb`',
      '`audio/theme.flac` `archive/export.7z` `database/run.sqlite`',
    ].join('\n'))).toEqual([
      'data/results.csv', 'design/layout.sketch', 'model/scene.glb',
      'audio/theme.flac', 'archive/export.7z', 'database/run.sqlite',
    ])
  ...
  it('recognizes intentional Markdown links and standalone paths without scanning prose', () => {
    expect(artifactDeliverableReferences([
      '[打开报告](output/report.pdf)',
      '[有空格的报告](output/My%20Report.pdf)',
      '- exports/final-deck.pptx',
      'C:\\work\\tables\\results.xlsx',
      'A plain sentence mentions output/ignored.pdf but is not an artifact declaration.',
      '[网页](https://example.com/report.pdf)',
      '[远程文件](file://server/share/report.pdf)',
    ].join('\n'))).toEqual([
      'output/report.pdf', 'output/My Report.pdf', 'exports/final-deck.pptx',
      'C:\\work\\tables\\results.xlsx',
    ])
  })

  it('normalizes local file links before handing them to the Host opener', () => {
    expect(artifactDeliverableReferences([
      '[macOS](file:///Users/test/output/report%20final.pdf)',
      '[Windows](file:///C:/work/deck.pptx)',
      '[Sandbox](sandbox:/mnt/data/export%20final.pdf)',
    ].join('\n'))).toEqual([
      '/Users/test/output/report final.pdf', 'C:/work/deck.pptx',
      '/mnt/data/export final.pdf',
    ])
  })
})
...
describe('plugin registration', () => {
  it('registers the tail entry and fiber disposal removes it', async () => {
    ...
    // A turn with neither mutation locations nor artifact references yields no vocabulary.
