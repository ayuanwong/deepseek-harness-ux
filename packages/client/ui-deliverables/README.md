...
`deliverablesDefinition` folds each Turn's successful mutation calls into engine-published `DeliverablesTurnData`; `producedForClosing` reads that data with the closing Assistant seq. A mutation is recognized by render intent, not tool name — a diff card, or a generic card whose `kind` is `edit` (the shape `str_replace_editor`'s insert presents) — so a new mutation tool joins by declaring what it does. Reads, deletes, and failed calls contribute nothing. Common command-generated artifacts that do not emit a mutation location — PDF, office documents, tables and datasets, images and design files, audio and video, archives, ebooks, databases, and 3D/CAD files — join when the settled closing answer intentionally declares an exact path as Markdown inline code, a Markdown link destination, or a path-only line. Local `file://` link destinations normalize back to Host paths. Web URLs, shell commands, fenced examples, globs, and paths embedded in unrelated prose remain excluded; paths keep first-seen order and are deduplicated. This supplement is presentation-only and issues no model request.
...
The closing prose carries the same vocabulary. This plugin provides the `chatFileMentions` service the chat view consults per closing message: `producedFileMentions` resolves an inline-code token by exact path, or by being exactly the basename of exactly one produced path — a basename two paths share stays inert rather than guessing. A resolved mention keeps its code chip and takes the markdown sheet's link language — link-blue at rest, underlined on hover, exactly like URL-promoted inline code — with the full path as its `title`; mentions never render inside anchors or streaming text. Decision record: the [inline file mentions Agent Note](../../../.agents/notes/implemented/feature/2026-08-07-web-inline-file-mentions.md).
...
- **Closing-answer supplements are not filesystem claims.** Common artifacts intentionally declared in accepted Markdown or on a path-only line are shown without an extra stat request; if the model names a path it did not create, the chip can hand an absent file to the Host opener. Mutation-tool locations remain the authoritative path for text and code outputs.
