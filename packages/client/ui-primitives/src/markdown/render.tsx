...
/** Display-only replacement for one parsed heading at a stable source offset. */
export interface MarkdownHeadingReplacement {
  /** Source offset of the heading node inside this Markdown document. */
  readonly offset: number
  /** Plain text rendered in place of the authored heading children. */
...
export interface MarkdownRenderContext {
  ...
  /** Settled-only presentation replacements keyed by heading source offset. */
  readonly headingReplacements: ReadonlyMap<number, string> | undefined
...
function renderNode(node: Md.RootContent, key: Key, context: MarkdownRenderContext): ReactNode {
  switch (node.type) {
    ...
    case 'heading': {
      ...
      const replacement = offset === undefined ? undefined : context.headingReplacements?.get(offset)
      return createElement(
        `h${node.depth}`,
        { key },
        ...(replacement === undefined ? renderChildren(node.children, context) : [replacement]),
      )
    }
    case 'blockquote':
      return (
