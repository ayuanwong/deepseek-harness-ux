...
export type { MarkdownCodeLabels, MarkdownFileMentions, MarkdownHeadingReplacement } from './render.tsx'
...
function renderSettled(
  ...
  headingReplacements: readonly MarkdownHeadingReplacement[] | undefined,
): ReactNode[] {
  ...
  const context: MarkdownRenderContext = {
    ...
    codeLabels,
    fileMentions,
    headingReplacements: headingReplacements === undefined
      ? undefined
      : new Map(headingReplacements.map(replacement => [replacement.offset, replacement.title])),
    targets,
    footnoteOrder: [],
...
export const MarkdownText = memo(function MarkdownText({
  text, streaming = false, codeLabels, fileMentions, headingReplacements,
}: {
  text: string
  streaming?: boolean
  codeLabels?: MarkdownCodeLabels | undefined
  fileMentions?: MarkdownFileMentions | undefined
  /** Settled-only visual labels; source Markdown and copy text stay untouched. */
  headingReplacements?: readonly MarkdownHeadingReplacement[] | undefined
}) {
  ...
  const children = useMemo(() => {
    if (!streaming) {
      ...
      return renderSettled(text, codeLabels, fileMentions, headingReplacements)
    }
  ...
  }, [text, streaming, codeLabels, fileMentions, headingReplacements])
