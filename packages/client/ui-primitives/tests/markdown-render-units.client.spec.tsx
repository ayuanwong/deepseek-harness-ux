...
describe('renderBlocks over hand-built trees', () => {
  it('replaces only a rendered heading label and preserves the source text', () => {
    const source = '# You are right, where is the problem?\n\nThe problem is empty hierarchy.'
    const { container } = render(
      <MarkdownText
        text={source}
        streaming={false}
        headingReplacements={[{ offset: 0, title: 'The hierarchy lacks information' }]}
      />,
    )
    expect(container.querySelector('h1')?.textContent).toBe('The hierarchy lacks information')
    expect(source).toContain('You are right, where is the problem?')
    expect(container.textContent).toContain('The problem is empty hierarchy.')
  })
