...
describe('tsdown client artifact', () => {
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    // Mount the registry through Cordis so declaration-aware injections run
    // with the same caller context as the production client runtime.
    ...
    // entry, so the binding stays deliberately empty. Locale behavior has its
    // own product tests; this artifact probe only needs the plugin-facing
    // dictionary and translate seats.
