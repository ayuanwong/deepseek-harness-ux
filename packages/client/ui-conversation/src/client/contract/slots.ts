...
import type {
  ProcessStagePresentationRequest, ProcessStagePresentationResult, ResponseHeadingReplacement,
...
export interface TurnTailOwnerProps {
  ...
  seq: number
  /** Visible text from the closing answer, used only for presentation-side
   * file references; it never re-enters the prompt or execution strategy. */
  closingText: string
  /**
...
export interface ChatNodeOwnerProps {
  ...
  /** Resolve cached display-only headings for one finalized answer. */
  refineResponseHeadings: (messageId: MessageId) => Promise<readonly ResponseHeadingReplacement[]>
}
...
export interface ChatViewInjected {
  ...
  /** Resolve cached display-only headings for one finalized answer. */
  refineResponseHeadings: (messageId: MessageId) => Promise<readonly ResponseHeadingReplacement[]>
  /** Summarize newly logged activity without entering the Agent's request or history. */
  refineProcessStage: (
    request: Omit<ProcessStagePresentationRequest, 'sessionId'>,
  ) => Promise<ProcessStagePresentationResult | null>
}
