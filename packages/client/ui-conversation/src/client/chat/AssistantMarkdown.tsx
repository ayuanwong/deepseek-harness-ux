...
export interface AssistantMarkdownProps {
  ...
  /** Display-only heading labels; source blocks and copy actions remain original. */
  headingReplacements?: readonly ResponseHeadingReplacement[] | undefined
...
export const AssistantMarkdown = memo(function AssistantMarkdown({
  blocks, streaming, interrupted, loadImage, mentions, headingReplacements, t,
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    switch (block.kind) {
      case 'text':
        {
          const replacements = headingReplacements
            ?.filter(replacement => replacement.textBlock === textBlock)
          ...
          rendered.push(
            <MarkdownText
              ...
              headingReplacements={replacements}
            />,
          )
          textBlock += 1
        }
