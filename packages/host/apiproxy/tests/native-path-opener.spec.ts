...
describe('browser-renderable documents', () => {
  ...
  it.each(['report.pdf', 'deck.pptx', 'table.xlsx', 'image.png', 'archive.zip'])(
    'leaves %s to its default application',
    async (name) => {
      const calls: string[][] = []
      const run = async (command: string, args: readonly string[]) => {
        calls.push([command, ...args])
      ...
      await openNativePath(`/w/${name}`, new AbortController().signal, { platform: 'darwin', run })
      // No LaunchServices read: only browser documents need a browser override.
      expect(calls).toEqual([['open', `/w/${name}`]])
    },
  )
