...
| `ctx.webPresentation` | `core` | [`web-presentation`](../packages/web/web-presentation) | - | `ui-conversation` | - | Projects bounded Session-log facts into optional process-stage labels and finalized answer-heading replacements through isolated auxiliary LLM requests; it never mutates model input, authored messages, or derived history. |
