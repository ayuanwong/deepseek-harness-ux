...
<a id="ctxwebpresentation--webpresentationservice"></a>

### `ctx.webPresentation` — `WebPresentationService`

Host Remote service. Both methods are presentation-only: they dispatch independently after the main request state exists, and their event record has no surface marker, so derived model history is unchanged.
 ...
 */
@Remote('response-headings') async responseHeadings(request: ResponseHeadingPresentationRequest): Promise<ResponseHeadingPresentationResult>
