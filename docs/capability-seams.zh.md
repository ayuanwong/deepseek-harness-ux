...
flowchart LR
  ...
  pkg_web_presentation["web-presentation"]
  svc_webPresentation["ctx.webPresentation<br/>Display-only Web presentation refinement"]
  pkg_ui_conversation["ui-conversation"]
  ...
  pkg_web_presentation --> svc_webPresentation
  ...
  svc_webPresentation --> pkg_ui_conversation
...
| `ctx.webPresentation` | `core` | [`web-presentation`](../packages/web/web-presentation) | - | `ui-conversation` | - | 将有界的 Session 日志事实投影为可选的过程阶段标签和已完成答案标题替换；辅助 LLM 请求相互隔离，绝不修改模型输入、原始消息或派生历史。 |
