...
describe('web e2e: empty-draft Cmd+Enter steers the whole queue', () => {
  ...
  it.skipIf(MODE === 'record')('queues two messages, then flushes both with an empty-draft Cmd+Enter', async () => {
    ...
    expect(await page.locator('[data-queue-dock]').count()).toBe(0)
    // The unified live process owns the reasoning and steering handoff. Wait
    // for its semantic log instead of reaching into the collapsed raw rows.
    await page.locator('[data-process-panel="running"] [role="log"]').waitFor({ timeout: 10_000 })
