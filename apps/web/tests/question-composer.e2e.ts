...
const PROCESS_EXPECTED = join(SNAPSHOT_DIR, 'process.expected.md')
...
const EXPLANATION = 'Your choice will change the recommendation, so first tell me which color direction fits you best.'
...
function narratedQuestionChunks(): StreamChunk[] {
  const callId = CallId('call_process_explanation')
  const reasoning = 'I should explain why the choice matters before asking.'
  return [
    { type: 'block-start', index: 0, blockType: 'reasoning' },
    { type: 'reasoning-delta', index: 0, text: reasoning },
    { type: 'block-start', index: 1, blockType: 'text' },
    { type: 'text-delta', index: 1, text: 'Your choice will change the recommendation, ' },
    { type: 'text-delta', index: 1, text: 'so first tell me which color direction fits you best.' },
    { type: 'block-start', index: 2, blockType: 'tool-call' },
    {
      type: 'tool-call-delta', index: 2, id: callId,
      name: 'ask_user_question', argumentsDelta: QUESTION_ARGS,
    },
    { type: 'block-end', index: 0, block: { type: 'reasoning', text: reasoning } },
    { type: 'block-end', index: 1, block: { type: 'text', text: EXPLANATION } },
    {
      type: 'block-end', index: 2,
      block: { type: 'tool-call', id: callId, name: 'ask_user_question', arguments: QUESTION_ARGS },
    },
    { type: 'usage', usage: { inputTokens: 128, outputTokens: 64 } },
...
describe('web e2e: resident question composer round trip', () => {
  ...
  beforeAll(async () => {
    if (MODE !== 'record') {
      replayDir = await mkdtemp(join(tmpdir(), 'dsh-question-process-replay-'))
      const replayOverride = join(replayDir, 'replay.override.json')
      const override: ReplayOverrideDoc = {
        patches: [{ at: 0, entry: { kind: 'chunks', chunks: narratedQuestionChunks() } }],
      }
      await writeFile(replayOverride, JSON.stringify(override))
      scaffold = await launchWebScaffold({ replayFixture: FIXTURE, replayOverride, paceMs: 15 })
    } else {
  ...
  it('asks through the composer, answers, and completes with the answer logged', async () => {
    ...
    // Intermediate explanation stays in one live process result. It must not
    // render as loose Assistant prose first and then jump into the blue rail.
    if (MODE !== 'record') {
      const liveProcess = page.locator('[data-process-panel="running"]')
      await liveProcess.waitFor({ timeout: 10_000 })
      await expect.poll(
        () => liveProcess.locator('[data-process-result]').getByText(EXPLANATION, { exact: true }).count(),
        { timeout: 10_000 },
      ).toBe(1)
      expect(await page.getByText(EXPLANATION, { exact: true }).count()).toBe(1)
    }
    ...
    if (MODE !== 'record') {
      ...
      const processSnapshot = await captureStableAria(page, '[data-process-panel="running"]', scaffold.workspaceCwd)
      await compareOrRefreshGolden(PROCESS_EXPECTED, processSnapshot, MODE)
    }
    ...
    const completedProcess = page.locator('[data-process-panel="done"]').first()
    await completedProcess.waitFor({ timeout: 10_000 })
    expect(await completedProcess.evaluate(element => (element as HTMLDetailsElement).open)).toBe(false)
