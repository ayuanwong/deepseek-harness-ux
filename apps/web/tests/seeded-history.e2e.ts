...
import {
  assertFixtureInventory, captureStableAria, collapseCompletedProcesses,
  compareOrRefreshGolden, expandCompletedProcesses, fixtureUserPrompts,
  launchWebScaffold, realizeSeedFixture, recordFixture, seedSession, watchConsole,
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('lists the seeded session cold and renders its history from the log', async () => {
    ...
    await expandCompletedProcesses(page)
  ...
  it.skipIf(MODE === 'record')('matches the historical conversation aria golden', async () => {
    ...
    await collapseCompletedProcesses(page)
