...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  const oldTextZh = '探索未至之境'
  const sourceNeedle = "'hero.headline': 'Into the Unknown'"
  const sourceNeedleZh = "'hero.headline': '探索未至之境'"
  const newText = `HMR UPDATED ${'x'.repeat(80)}`
  const updatedSource = originalSource.toString()
    .replace(sourceNeedle, `'hero.headline': '${newText}'`)
    .replace(sourceNeedleZh, `'hero.headline': '${newText}'`)
  if (updatedSource === originalSource.toString()) {
    throw new Error(`HMR source lacks ${JSON.stringify(sourceNeedle)} and ${JSON.stringify(sourceNeedleZh)}`)
  }
  ...
  try {
    ...
    await page.getByText(new RegExp(`^(?:${oldText}|${oldTextZh})$`)).waitFor({ timeout: 15_000 })
