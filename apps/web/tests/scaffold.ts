...
/** Open settled process groups before a scenario inspects technical rows. */
export async function expandCompletedProcesses(page: Page): Promise<void> {
  const disclosures = page.locator('details[data-process-panel]:not([open]) > [data-process-disclosure]')
  for (let attempt = 0; attempt < 500; attempt += 1) {
    if (await disclosures.count() === 0) {
      await page.waitForTimeout(50)
      if (await disclosures.count() === 0) return
    } else {
      await disclosures.first().click()
    }
  }
  throw new Error('completed process disclosures did not finish expanding')
}

/** Restore the product-default collapsed state after inspecting process rows. */
export async function collapseCompletedProcesses(page: Page): Promise<void> {
  const disclosures = page.locator('details[data-process-panel="done"][open] > [data-process-disclosure]')
  for (let attempt = 0; attempt < 500; attempt += 1) {
    if (await disclosures.count() === 0) return
    await disclosures.first().click()
  }
  throw new Error('completed process disclosures did not finish collapsing')
}

/** Open the latest process and its technical rows for interaction-focused scenarios. */
export async function expandLatestProcessDetails(page: Page): Promise<void> {
  const panel = page.locator('details[data-process-panel]').last()
  await panel.waitFor({ timeout: 15_000 })
  if (!await panel.evaluate(element => (element as HTMLDetailsElement).open)) {
    await panel.locator(':scope > [data-process-disclosure]').click()
  }
  const details = panel.locator('details[data-process-live-disclosure]').first()
